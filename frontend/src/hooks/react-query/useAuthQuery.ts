...
export const AuthApis = {
  ...
  checkEmailDuplicate: async (userInfo: UserInfo): Promise<boolean> => {
    const response = await AuthApis.instance.get(
      `/signup/exists-email/${userInfo.email}`,
      {
  ...
  checkUsernameDuplicate: async (userInfo: UserInfo): Promise<Boolean> => {
    const response = await AuthApis.instance.get(
      `/signup/exists-username/${userInfo.username}`,
      {
  ...
  signin: async (userInfo: UserInfo) => {
    ...
    const Refreshtoken = rawRefreshToken
      ? rawRefreshToken.replace(/^Bearer\s+/i, '')
      : null;
      ...
      async (error) => {
        ...
        if (error.response.status === 401 && !originalRequest._retry) {
          ...
          try {
            ...
            const newAccesstoken = rawAccessToken
              ? rawAccessToken.replace(/^Bearer\s+/i, '')
              : null;
            const newRefreshtoken = rawRefreshToken
              ? rawRefreshToken.replace(/^Bearer\s+/i, '')
              : null;
  ...
  fetchData: async (endpoint: string, type: string, data?: any) => {
    ...
    switch (type) {
      case 'get':
        response = await AuthApis.instance.get(
          `http://localhost:8001/user-service/${endpoint}`,
          {
            headers: {
      ...
      case 'post':
        response = await AuthApis.instance.post(
          `http://localhost:8001/user-service/${endpoint}`,
          data,
          {
            headers: {
